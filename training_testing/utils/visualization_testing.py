...
from config.config_testing import TESTING_OUTPUT_GRAPH, TESTING_INPUT_GRAPH, NETWORKS_LIST_GRAPH, GRAPH_MODE, TESTING_OUTPUT_DIR
...
def gather_data_ca_size(pres):
    for presentation in pres:
        dfS_audi = read_tca_files(TESTING_INPUT_GRAPH, "Audi", presentation)
        dfS_arti = read_tca_files(TESTING_INPUT_GRAPH, "Arti", presentation)
    ...
    for pres in NETWORKS_LIST_GRAPH:
        ...
        if "auditory" in GRAPH_MODE or "articulatory" in GRAPH_MODE:
            for cond in ["Audi", "Arti"]:
                dfS = read_tca_files(TESTING_INPUT_GRAPH, cond, pres)
